...
if(isset($_POST['action']) && isset($_POST['tmdbID']) && $_POST['action'] == 'delete') {
  $tmdbID = $_POST['tmdbID'];
  if($conn->query("DELETE FROM media WHERE tmdbID = '$tmdbID'")) {
    if($conn->query("DELETE FROM wish_list WHERE mediaID = '$tmdbID'")){
      if($conn->query("DELETE FROM watch_list WHERE mediaID = '$tmdbID'")) {
        ...
        echo 'Error Deleting Movie From Watchlist';
        exit();
      }
    } else {
      echo 'Error Deleting Movie From Wishlist';
      exit();
    }
  } else {
...
if(isset($_POST['submit'])) {
  ...
  if($conn->query($sql)) {
    ...
    header('Location: edit_movie.php?tmdbID='.$tmdbID);
    exit();
  } else {
    ...
    if(isset($_SESSION['_success_msg'])) {      
      ...
      <form method="POST"> 
        ...
        <div class="right-col">
            ...
            <div id="dropdown1">
              <?php $res = $conn->query("SELECT * FROM genre ORDER BY name");
